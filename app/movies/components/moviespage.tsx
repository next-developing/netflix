...
export function MoviesPageClient({ initialData, initialPage, movie }: Props) {
  ...
  const { data = initialData, isLoading } = useSWR(
    `/api/moviestrue?page=${currentPage}`,
    fetcher,
    {
  ...
  useSWR(
    currentPage > 1 ? `/api/moviestrue?page=${currentPage - 1}` : null,
    fetcher,
    { revalidateOnFocus: false }
  );
  
  useSWR(
    currentPage < data.total_pages ? `/api/moviestrue?page=${currentPage + 1}` : null,
    fetcher,
    { revalidateOnFocus: false }
  );
  ...
  <>
    ...
    <div className="  w-full h-[70vh] mb-8 overflow-hidden ">
      <img
        ...
        src={`https://image.tmdb.org/t/p/original${data.results[0].backdrop_path || data.results[0].poster_path}`}
