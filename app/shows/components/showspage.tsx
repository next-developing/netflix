...
import { Series, SeriesData } from '@/lib/types/datatypes';
...
export function ShowsPageClient({ initialData, initialPage }: Props) {
  ...
  useSWR(
    currentPage > 1 ? `/api/shows?page=${currentPage - 1}` : null,
    fetcher,
    { revalidateOnFocus: false }
  );
  
  useSWR(
    currentPage < data.total_pages ? `/api/shows?page=${currentPage + 1}` : null,
    fetcher,
    { revalidateOnFocus: false }
  );
      ...
      <div className="absolute   left-8 max-w-xl text-white md:pl-20 top-75 transform -translate-y-1/2">
        ...
        <div className="flex items-center gap-3 mb-5">
          <span className="inline-block  text-gray-300 text-sm px-3 py-1 rounded ">
            ⭐   {data.results[0].vote_average ? data.results[0].vote_average.toFixed(1) : 'N/A'}  {/* ✅ Правильно */}
          </span>
        ...
        <div className="flex">
          <Link 
            href={`/titleseries/${data.results[0].id}`}
    ...
    ⭐ {movie.vote_average ? movie.vote_average.toFixed(1) : 'N/A'}
      ...
      <div className="flex justify-center gap-4 mt-6">
        <button
          ...
          className="px-4 py-2 bg-gray-800/50 border-gray-500 border-1 text-white rounded-xl disabled:hidden hover:bg-gray-600 cursor-pointer transition-colors"
        >
          ← Previous
        </button>
              ...
              <button
                ...
                className={`px-3 py-1 rounded ${
                  pageNum === currentPage 
                    ...
                    : 'text-gray-600 hidden md:inline hover:bg-gray-300'
                }`}
              >
                {pageNum}
              </button>
        ...
        <button
          ...
          className="px-4 py-2 bg-gray-800/50 border-gray-500 border-1 text-white rounded-xl disabled:bg-gray-500 hover:bg-gray-600 cursor-pointer transition-colors"
        >
          Next →
        </button>
      </div>
